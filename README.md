...
|Password visible icons|Display password on hover|
...
* Adding recipe modal: if a required field was not populated when trying to submit there were no msgs to tell the user to populate field before submitting - the temporary solution was to remove certain fields as "required". The longer-term solution would be to create simple javascript code that checks which required field was no populated and feed that back to the user on-screen.

* If the user attempts to upload recipe image before first selecting a file, there is no feedback to the user to select a file first
